import os, re, urllib, urllib2, ipdb
import markdown2
            ...
            if filename.lower() == "readme.md" and len(dirs) != 0 and path != folder:
                ...
                with open(os.path.join(path,filename), 'rb') as readme:
                    ...
                    section = {
                        ...
                        "slug": urllib.quote(section_name),
                        "examples": []
                    }
...
def get_section(section_name):
    section = {'name': section_name, 'slug': urllib.quote(section_name)}
    examples = []
    print section_name
            ...
            if filename.lower() == "readme.md" and path != os.path.join(folder, section_name): # and len(dirs) != 0:
                ...
                with open(os.path.join(path,filename), 'rb') as readme:
                    ...
                    example = {
                        "name": example_name,
                        "description": description,
                        "slug": urllib.quote(example_name)
                    }
...
def get_example(section_name, example_name):
    section = {'name': section_name, 'slug': urllib.quote(section_name)}
    readme = {'name': example_name, 'slug': urllib.quote(example_name)}
    examples = []
